...
class EnrollBloc extends Bloc<EnrollEvent, EnrollState> {
  ...
  Future<void> _onEnrollInCourse(
    ...
    try {
      ...
      emit(const EnrollError('Server error. Please try again later.'));
    } on HttpFailure {
      emit(const EnrollError('Network error. Please check your connection.'));
    } on NoDataFailure {
      emit(const EnrollError('No data available.'));
    } catch (e) {
      emit(const EnrollError('Failed to enroll in course'));
    }
  }
}
//
